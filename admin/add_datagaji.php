...
$bulan_gaji = isset($_POST['modal_bulan_gaji']) ? $_POST['modal_bulan_gaji'] : '';
$tahun_gaji = isset($_POST['modal_tahun_gaji']) ? $_POST['modal_tahun_gaji'] : '';
$id_transaksi_karyawan = isset($_POST['id_transaksi_karyawan']) ? $_POST['id_transaksi_karyawan'] : '';
...
if ($id_transaksi_karyawan && $bulan_gaji && $tahun_gaji) {
    ...
    // Menangani error jika ada masalah dengan database atau query
    if ($result) {
        $data = mysqli_fetch_assoc($result); // Ambil hasil data karyawan dan gaji
        
        // Membuat respons data
        if ($data) {
            $response = array(
                'success' => true,
                'data' => array(
            ...
            $response = array('success' => false, 'message' => 'Data tidak ditemukan.');
        }
    } else {
        // Jika ada error database, kirimkan pesan error dalam format JSON
        $error_message = 'Database error: ' . mysqli_error($koneksi);
        $response = array('success' => false, 'message' => $error_message);
    }

    echo json_encode($response); // Pastikan ini menghasilkan JSON yang valid
}
?>
